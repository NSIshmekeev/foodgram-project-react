...
```
git clone https://github.com/NSIshmekeev/foodgram-project-react.git
```
...
* После успешного развертывания проекта на удаленном сервере, можно выполнить:
    ...
    ```
    docker compose exec -it backend python manage.py load_ingredients
    ```
    - Импортровать в БД теги, чтобы пользователи могли ими пользоваться при создании рецептов:  
    ```
    docker compose exec -it backend python manage.py load_tags
    ```
